<div align="center">

# Helano Pessoa

**Operations Research · Mathematical Optimization · Data Science**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-helano--pessoa-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/helano-pessoa)
[![Email](https://img.shields.io/badge/Email-helanopessoa1903%40gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:helanopessoa1903@gmail.com)

</div>

## 🇧🇷 Sobre mim

Sou Cientista de Dados na **GOL Linhas Aéreas**, com mais de seis anos de experiência em Pesquisa Operacional, otimização matemática, ciência de dados e desenvolvimento de soluções de apoio à decisão.

Minha experiência inclui modelos de otimização, simuladores, previsão de demanda e aplicações analíticas nos setores de aviação, logística, energia, varejo, finanças, alimentos e metalurgia. Sou bacharel em Matemática Industrial, mestrando em Pesquisa Operacional na Universidade Federal do Ceará e aluno do MBA em Advanced Analytics and Business Optimization na Universidade Federal do Paraná (UFPR).

Tenho interesse especial em transformar problemas operacionais em modelos matemáticos, implementações reproduzíveis e decisões que possam ser explicadas às áreas de negócio.

## 🇺🇸 About me

I am a Data Scientist at **GOL Linhas Aéreas**, with more than six years of experience in Operations Research, mathematical optimization, data science, and decision-support solutions.

My background includes optimization models, simulators, demand forecasting, and analytical applications across aviation, logistics, energy, retail, finance, food, and manufacturing. I hold a B.Sc. in Industrial Mathematics and I am currently pursuing an M.Sc. in Operations Research at the Federal University of Ceará and an MBA in Advanced Analytics and Business Optimization at the Federal University of Paraná (UFPR).

I am particularly interested in turning operational problems into mathematical models, reproducible implementations, and decisions that can be clearly communicated to business teams.

## Core expertise

| Area | Methods and tools |
|---|---|
| Operations Research | Linear, integer, mixed-integer and nonlinear optimization; routing; scheduling; resource allocation |
| Modeling | OR-Tools, Pyomo, Gurobi, CPLEX, JuMP, SciPy |
| Data Science | Python, SQL, Pandas, NumPy, scikit-learn, Nixtla |
| Data platforms | BigQuery, Databricks, PySpark, Airflow, Docker |
| Applications | FastAPI, Streamlit, Plotly, Power BI, Looker Studio |

## Selected public projects

| Project | Focus | Status |
|---|---|---|
| [Socratica puzzle](https://github.com/helano-pessoa/socratica-puzzle) | Mathematical modeling with Julia/JuMP | Earlier study; attributed challenge |
| [Knapsack app](https://github.com/helano-pessoa/knapsack-app) | Capacity-constrained selection | Earlier prototype |
| [A/B testing libraries](https://github.com/helano-pessoa/ab-testing-libraries) | Statistical experimentation | Study outline; experiments not yet versioned |

Curriculum vitae in Portuguese or English is available [on request](mailto:helanopessoa1903@gmail.com). Project demonstrations use synthetic or otherwise suitable public data; private professional work is not distributed here.

## Publications

- [Trajectory Modeling via Random Utility Inverse Reinforcement Learning](https://doi.org/10.1016/j.ins.2024.120128) — *Information Sciences*
- [Modelagem de Trajetórias de Veículos por meio de Aprendizado por Reforço Inverso e Teoria da Escolha Discreta](https://proceedings.science/anpet/anpet-2022/trabalhos/modelagem-de-trajetorias-de-veiculos-por-meio-de-aprendizado-por-reforco-inverso?lang=pt-br) — ANPET

---

<div align="center">

**Transformando problemas operacionais em decisões melhores com matemática.**

**Turning operational problems into better decisions through mathematics.**

</div>
